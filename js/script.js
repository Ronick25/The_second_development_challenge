...
$(document).ready(function() {
    ...
    $('.continue-search').on('click', function () {
        ...
        var search_algorithm = $("#search_algorithm").val();
        ...
        if (search_algorithm === "depth_first_search") {
            drawDepthFirstSearch(previous, current, start, last, stack, animation_speed, edge);
        } else {
            previous = current;
            queue.push(current)
            drawBreadthFirstSearch(previous, current, last, queue, animation_speed, edge);
        }
    });
...
function createMatrix(edge, coordinats) {
    $("#rarefaction").val() !== "" ? R = $("#rarefaction").val() : R = 50;
...
function drawDepthFirstSearch(previous, current, start, last, stack, animation_speed, edge) {
    setTimeout(function () {
        if (current === last) {
            alert('The end');
        } else {
            ...
            if (current !== start) {
                previous.color = "#c6e0b4";
                current.color = "green";
                $("#" + previous.i + "_" + previous.j).css('background-color', matrix[previous.i][previous.j].color, "!important");
                $("#" + current.i + "_" + current.j).css('background-color', matrix[current.i][current.j].color);
            }
            var next = checkNeigbors(current.i, current.j, edge);
            if (next) {
                stack.push(current);
                next.visited = true;
                previous = current;
...
function drawBreadthFirstSearch(previous, current, last, queue, animation_speed) {
    setTimeout(function () {
        console.log(current)
        ...
        if (queue.length >= 0) {
            ...
            previous.color = "#c6e0b4";
            current.color = "green";
            $("#" + previous.i + "_" + previous.j).css('background-color', matrix[previous.i][previous.j].color);
            $("#" + current.i + "_" + current.j).css('background-color', matrix[current.i][current.j].color);
            previous = current;
            ...
            if (childs) {
                childs.forEach(function (child) {
                    if (child.visited === false) {
                        queue.push(child)
                        child.visited = true;
                        drawBreadthFirstSearch(previous, current, last, queue, animation_speed)
                    }
                });
            }
        }
...
function checkNeigbors(i, j, edge) {
    ...
    if (transition_method === "four_steps") {
        ...
        getStrategyFourSteps(directions);
    } else {
        var top_right = index(i - 1, j + 1, edge);
        var bottom_right = index(i + 1, j + 1, edge);
        var bottom_left = index(i + 1, j - 1, edge);
        var top_left = index(i - 1, j - 1, edge);

        var directions = [top, top_right, right, bottom_right, bottom, bottom_left, left, top_left];
        getStrategyEightSteps(directions);
    }
...
function getStrategyFourSteps(directions) {
...
function getStrategyEightSteps(directions) {
    var strategy = $("#strategy").val();
...
function getStartAndEndCells() {
    ...
    var end = $("#end-coordinat").val().split(",", 2);
    var coordinats = [];
